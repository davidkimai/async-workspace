...
This folder implements the workflow described in `async_workflow.pdf`.
...
3. Write in your own area:
   - Add transcripts, references, and notes only under your folder:
     - `workspace/elizabeth/transcripts & notes`
     - `workspace/jason/transcripts & notes`
     - `workspace/amy/transcripts & notes`
     - `workspace/hayley/transcripts & notes`
...
5. Draft cycle in this workspace:
   ...
   - Push your branch and merge via PR once reviewed.
...
Use `workspace/render-status.sh` to generate/update the PDF after each merge.